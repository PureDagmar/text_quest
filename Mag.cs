...
namespace tekstquestconsol
{
    class Mag : Character
    {
        ...
        public string magic;       //поле для определения навыка персонажа в игре

        //Конструктор класса
        public Mag(string name, string weapon) : base()
        {
            this.name = name;
            this.weapon = weapon;
            this.lives = 5;
            this.magic = "магия стихий";
        }
        ...
        public override void check_object()
        {
            ...
            {
                Console.WriteLine("... зуб дракона. Ты не заметил, как оказался прямо в его пасти! Ты теряешь жизнь, как только его челюсти смыкаются над твоей магической головой!");
                this.lives -= 1;
        ...
        public override void run_away()
        {
            ...
            if (result % 2 == 1)
            {
                Console.WriteLine($"Маг, тебе удается остаться незамеченным и замести за собой следы");
            }
            else
            {
                Console.WriteLine($"Ты в ужасе пытаетешься сбежать от врага! Тебе это удается, но он успел тебя ранить! Ты теряешь жизнь!");
                this.lives -= 1;
                string lives = this.lives.ToString();
                Console.WriteLine($"У тебя осталось {lives} жизней ");
            }
        ...
        public override void fight()
        {
            ...
            if (result % 2 == 1)
            {
                Console.WriteLine($"Тебе удалось победить врага с помощью твоей {this.magic}! Можем двигаться дальше!");
            }
            else
            {
                Console.WriteLine($"Твоя {this.magic} была повержена! Ты потерял жизнь!");
                this.lives -= 1;
                string lives = this.lives.ToString();
                Console.WriteLine($"У тебя осталось {lives} жизней ");
            }
