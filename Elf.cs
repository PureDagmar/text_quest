...
namespace tekstquestconsol
{
    class Elf : Character
    {
        ...
        public string speed;       //поле для определения навыка персонажа в игре

        //Конструктор класса
        public Elf(string name, string weapon) : base()
        {
            this.name = name;
            this.weapon = weapon;
            this.lives = 6;
        ...
        public override void check_object()
        {
            ...
            {
                Console.WriteLine("... зуб дракона. Ты не заметил, как оказался прямо в его пасти! Ты теряешь жизнь, как только его челюсти смыкаются над твоей эльфийской головой!");
                this.lives -= 1;
            ...
            else if (result == 3)
            {
                ...
                this.lives += 1;
                string lives = this.lives.ToString();
                Console.WriteLine($"У Вас стало {lives} жизней ");
            }
            else if (result == 4)
            {
        ...
        public override void run_away()
        {
            ...
            if (result % 2 == 1)
            {
                Console.WriteLine($"Эльф, тебе удается остаться незамеченным и замести свои следы");
            }
            else
            {
        ...
        public override void fight()
        {
            ...
            if (result % 2 == 1)
            {
                Console.WriteLine($"Тебе удалось победить врага с помощью твоей скрытой способности - {this.speed}! Можешь быстро двигаться дальше!");
            }
            else
            {
                Console.WriteLine($"Твоя {this.speed} была замедлена! Ты потерял жизнь!");
                this.lives -= 1;
                string lives = this.lives.ToString();
                Console.WriteLine($"У тебя осталось {lives} жизней ");
            }
